...
class IngredientAnalyser:
    ...
    def save_in_dict(self):
        ...
        amounts_for_10: list [str] = self.recipereader.get_ingredients_dict()['10 p']
        for i in range(0, len(ingr_names)):
            amount, unit = self.split_ingredient_amounts_and_units(amounts_for_10[i])
            if not self.ingredient_already_added(ingr_names[i]):
                ingr_dict = {'ingredient': ingr_names[i], 'amount': amount, 'unit': unit}
        ...
        for ingr in self.ingredients:
            if ingr['ingredient'].lower() == ingredient.lower():
                if ingr['unit'].lower() == unit.lower():
                    ...
                    print("Could not add", ingredient, amount, unit, "to", ingr['ingredient'], ingr['amount'], ingr['unit'], "because unit did not match")
    ...
    def convert_units(self):
        ...
        for item in self.ingredients:
            if item['ingredient'].lower() in spices: #spices are always ordered in a certain amount
                item['amount'] = 100
                item['unit'] = "g"
