   ...
   """
    ...
    def read(self):
        ...
        try:
            ...
            # first table is not a recipe
            for index, table in enumerate(document.tables[1:]):
                list_from_table = []
                for row in range(len(table.rows)):
                    for col in range(len(table.columns)):
                        # this string starts recipe
                        if not "Ingredients" in table.cell(row, col).text:
    ...
    def save_ingredients_for_ten(self, list_from_table: list):
        ...
        if len(ingredients_names) < len(ingredient_amounts_for_ten):
            ingredient_amounts_for_ten = ingredient_amounts_for_ten[:len(
                ingredients_names)]
            ...
            if ingredient_amounts_for_ten[i].strip() != 'pinch'\
                ...
                if ingredients_names[i].lower().strip() != 'water' \
                    ...
                    if not self.ingredient_already_in_ingredients_list(ingredients_names[i]):
                        ...
                        ingr_dict = {'ingredient': ingredients_names[i].strip(
                        ...
                        if ingredient_amounts_for_ten[i].strip() != 'pinch'\
                            ...
                            self.add_to_existing_ingredient(
                                ingredients_names[i], amount, unit)
