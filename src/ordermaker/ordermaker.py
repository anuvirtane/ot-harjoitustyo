class OrderMaker:
    """Has ingredients to be ordered in self.ingredients list with dicts in it. Saves ingredients to it when given a list of the same format.
    ...
    def add_ingredients(self, ingredients_list: list):
        '''Ingredients come in amounts calculated for ten. When added, they are also multiplied to match wished amount.'''
        for item in ingredients_list:
            if not self.dict_already_in_ingredients(item):
                self.ingredients.append({'ingredient': item['ingredient'], 'amount': item['amount']* self.multiplier, 'unit': item['unit']})
            else:
